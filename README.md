...
|         $d_k$         | In the process of creating the Attention matrix, there is a hidden dimension between the queries and the keys; this hidden dimension is $d_k$. That is, outside of the Attention module changing $d_k$ will have no visible effect (that is, the output dimensions will not change, nor is $d_k$ constrained by $h$ or $d_{model}$ hyperparameters). Increasing $d_k$, however, will allow the Attention matrix computation to be more expressive / happen in a higher dimension. $d_k$ is commonly understood to be set to $d_{model} / h$, and so is not always listed as a separate hyperparameter. |
|         $d_v$         |                                                                                                                         The embedding dimension of the Values within the Attention module, which will appear as the final output dimension of every Attention Module ($B$, $L$, $d_v$). That is, $d_v$ defines the expressivity of the Attention modules. $d_v$ is commonly understood to be set to $d_{model} / h$, and so is not always listed as a separate hyperparameter.                                                                                                                         |
