...
class ShakespeareDataset(Dataset):
    def __init__(self, data, block_size=8, batch_and_mask=False):
...
class ShakespeareDataModule(pl.LightningDataModule):
    def __init__(self, tokenizer=None, block_size=8, batch_size=1, num_workers=1, train_percent=0.8, val_percent=0.1):
        ...
        self.train_data = ShakespeareDataset(data=text[:m], block_size=block_size, batch_and_mask=True)
        self.val_data = ShakespeareDataset(data=text[m:m+n], block_size=block_size, batch_and_mask=True)
        self.test_data = ShakespeareDataset(data=text[m+n:], block_size=block_size, batch_and_mask=True)
