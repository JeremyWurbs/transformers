...
model = LightningModel(model, num_classes=dm.vocab_size, validation_hook=validation_hook)
logger = TensorBoardLogger(save_dir=os.getcwd(), name=f"{os.path.join('experiments', param.__str__())}")
trainer = Trainer(max_epochs=20, logger=logger, val_check_interval=1.0, strategy='ddp_find_unused_parameters_true', devices=[1])
